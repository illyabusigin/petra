...
func newApp(cfg appConfig) (*app, error) {
	...
	if cfg.Dev {
		...
		// Vite writes static/app.css in watch mode. Petra watches that folder
		// and tells the browser client to cache-bust the stylesheet in place.
	...
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		// The example keeps routing deliberately small so the Tailwind asset
		// integration remains the only moving part.
		if r.URL.Path != "/" {
			http.NotFound(w, r)
			return
		}
		if err := tmpl.ExecuteTemplate(w, "index", pageData{Title: "Petra Tailwind example", Dev: cfg.Dev}); err != nil {
